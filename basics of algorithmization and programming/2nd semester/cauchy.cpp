...
int main()
{
	...
	x = a; // ñòàâèì èêñ â íà÷àëî 

	y[0] = 2 * x; // îïðåäåëÿåì ïåðâóþ ôóíêöèþ
	y[1] = exp(x); // îïðåäåëÿåì âòîðóþ ôóíêöèþ
	...
	for (int n = 1; n <= num_of_h; n++)
	{
		...
		for (int i = 0; i < num_of_funcions; i++)
			y[i] += h * f[i]; // òóò ïî ôîðìóëå èç ìåòîäû
...
void functions(double x, double* y) //âû÷èñëåíèå ôóíêöèé
{
...
void output(double x, double* y) // âûâîä
{
